...
public class Image extends SymbolType implements Rule {
    ...
    private void addOptions(Symbol link, Map<String, String> options) {
        for (String key : options.keySet()) {
            ...
            if (key.equals("-m"))
                link.putProperty(Link.StyleProperty, String.format("%2$smargin:%1$spx %1$spx %1$spx %1$spx;", options.get(key), link.getProperty(Link.StyleProperty)));
            if (key.equals("-b"))
                link.putProperty(Link.StyleProperty, String.format("%2$sborder:%1$spx solid black;", options.get(key), link.getProperty(Link.StyleProperty)));
        }
    }
